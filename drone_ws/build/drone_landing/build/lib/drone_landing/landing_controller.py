...
Fixes:
  1. After landing: drone tracks platform XY (moves with it) but Z=0 cmd
  2. Prints "Successfully Landed" message clearly
  3. Publishes zero Z so drone stays on surface
  4. Still uses feedforward + PID for pre-landing
"""
import rclpy
...
class LandingController(Node):
    def __init__(self):
        ...
        self.pid_z = PIDController(kp=1.0, ki=0.03, kd=0.4, output_limit=1.5)
        ...
        self.plat_x  = 2.0; self.plat_y  = 0.0
        self.plat_vx = 0.0; self.plat_vy = 0.0
        ...
        self.approach_alt  = 4.0
        self.descend_alt   = 0.15
        self.descent_rate  = 0.2
        self.align_thresh  = 0.35
        self.landed_thresh = 0.40   # slightly relaxed
    ...
    def move_with_platform(self):
        """
        After landing: drone moves WITH the platform (same velocity),
        zero vertical. This keeps drone on platform surface as it moves.
        """
        cmd = Twist()
    ...
    def trigger_landing(self, actual_xy):
        ...
        self.get_logger().info(
            ...
            '╔══════════════════════════════════════════╗\n'
            '║      SUCCESSFULLY LANDED                 ║\n'
            f'║  Center error  : {actual_xy*100:5.1f} cm               ║\n'
            f'║  Final altitude: {self.drone_z:.3f} m               ║\n'
            f'║  Drone  pos    : ({self.drone_x:.2f}, {self.drone_y:.2f})        ║\n'
            f'║  Platform pos  : ({self.plat_x:.2f}, {self.plat_y:.2f})        ║\n'
            '╚══════════════════════════════════════════╝')

    def loop(self):
        # ── LANDED: move with platform XY, zero Z ────────────────── #
        if self.is_landed:
        ...
        if self.marker_detected and self.aruco_ox is not None:
            aruco_x  = self.drone_x + self.aruco_ox
            aruco_y  = self.drone_y + self.aruco_oy
            target_x = 0.7 * aruco_x + 0.3 * pred_x
            target_y = 0.7 * aruco_y + 0.3 * pred_y
        else:
            target_x = pred_x
            target_y = pred_y
        ...
        # Z setpoint
        if self.state in ('TAKEOFF', 'SEARCH', 'APPROACH'):
            err_z = self.approach_alt - self.drone_z
        elif self.state == 'DESCEND':
            if xy < self.align_thresh:
                target_z = max(self.descend_alt,
                               self.drone_z - self.descent_rate * dt)
            else:
        ...
        vz = self.pid_z.compute(err_z, self.drone_z, dt)
        ...
        e = Float32MultiArray()
        e.data = [err_x, err_y, err_z, xy, self.plat_vx, self.plat_vy, actual_xy]
        self.error_pub.publish(e)

        # Landing detection
        if (self.state == 'DESCEND'
                and self.drone_z <= self.descend_alt + 0.15
                and actual_xy < self.landed_thresh):
