...
state_machine.py
Fixes:
  1. Approach checks platform Z too (platform bobs in Z now)
  2. Better DESCEND trigger using actual PID xy_error
  3. After LANDED: platform keeps moving, drone rides it
  4. Logs "Successfully Landed" every 10s
"""
import rclpy
...
class StateMachine(Node):
    def __init__(self):
        ...
        self.platform_x=2.0; self.platform_y=0.0; self.platform_z=0.025
        self.xy_error   = 999.0
        self.z_gap      = 999.0   # drone altitude above platform surface
    ...
    def plat_cb(self, msg):
        ...
        self.platform_z = msg.pose.pose.position.z
    ...
    def errors_cb(self, msg):
        if len(msg.data) >= 8:
            ...
            self.z_gap    = msg.data[7]   # actual z gap above platform
    ...
    def run(self):
        ...
        if self.is_landed:
            ...
            # Platform keeps moving — drone rides it
        ...
        if self.current_state == State.IDLE:
            ...
            if dist < 3.0 and self.drone_z >= self.takeoff_alt - 0.5:
                self.get_logger().info(
                    f'Platform in range dist={dist:.2f}m — APPROACH')
            ...
            if dist > 5.0:
                self.get_logger().warn(
                    f'Lost platform dist={dist:.2f}m — SEARCH')
                self.search_start_time = time.time()
                self.transition(State.SEARCH); return

            # Use BOTH xy_error from PID AND direct dist as fallback
            # If either says we're close enough → DESCEND
            xy_close = (self.xy_error < self.approach_xy_thresh or
                        dist < 0.6)
            if xy_close:
                self.get_logger().info(
                    f'Aligned xy_error={self.xy_error:.2f}m '
                    f'dist={dist:.2f}m — DESCEND')
                self.transition(State.DESCEND)

        elif self.current_state == State.DESCEND:
            dist = self.dist_to_platform()
            # If drifted too far while descending, go back to approach
            if dist > 2.0 and self.drone_z > (self.platform_z + 1.0):
                self.get_logger().warn(
                    f'Drifted dist={dist:.2f}m — APPROACH')
